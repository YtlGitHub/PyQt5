...
class SignWindow2(QWidget):
    ...
    def set_ui(self):  # 集合所有的设计
        self.set_background_image()  # 添加背景图
        self.add_line_edit()  # 添加输入框
        self.add_button()  # 添加按钮
        self.add_label()  # 添加标签文字
    ...
    def add_label(self):
        ...
        self.YTL_label = QLabel(self)

        # 相应的标签设置文本
        self.username_label.setText("username")
        self.password_label.setText("password")
        self.confirm_label.setText("confirmed")
        self.YTL_label.setText("YTL--a python learner")
        ...
        self.YTL_label.setFixedSize(600, 40)
        ...
        self.YTL_label.setFont(label_font)
    ...
    def sign_up(self):
        ...
        if not password or not confirm:  # 如果有一个密码或者密码确认框为空
        ...
        elif self.database2.is_has_admin(username):  # 如果用户名已经存在
            ...
            if password == confirm and password:  # 如果两次密码一致，并且不为空
                if len(username) < 5:
                    QMessageBox.information(self, 'Error', 'The username is too short, change it for a long one, at least 5 words', QMessageBox.Yes, QMessageBox.Yes)
                    return
                if len(password) < 6:
                    QMessageBox.information(self, 'Error', 'You password\'s length is less than 6, please input again', QMessageBox.Yes)
                    return
                else:
                    self.database2.insert_table(username, password)  # 将用户信息写入数据库
                    QMessageBox.information(self, 'Successfully', 'Sign up successfully'.format(username), QMessageBox.Yes)
